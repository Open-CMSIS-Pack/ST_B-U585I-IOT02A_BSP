...
void MX_WIFI_SPI_TransferCallback(SPI_HandleTypeDef *hspi);
void MX_WIFI_SPI_ErrorCallback   (SPI_HandleTypeDef *hspi);
#endif
...
static int8_t MX_WIFI_SPI_Init(uint16_t mode)
{
  ...
  if (HAL_SPI_RegisterCallback(&MXCHIP_SPI, HAL_SPI_RX_COMPLETE_CB_ID, MX_WIFI_SPI_TransferCallback) != HAL_OK) {
    ret = MX_WIFI_STATUS_ERROR;
  }

  if (HAL_SPI_RegisterCallback(&MXCHIP_SPI, HAL_SPI_TX_COMPLETE_CB_ID, MX_WIFI_SPI_TransferCallback) != HAL_OK) {
    ret = MX_WIFI_STATUS_ERROR;
  }

  if (HAL_SPI_RegisterCallback(&MXCHIP_SPI, HAL_SPI_TX_RX_COMPLETE_CB_ID, MX_WIFI_SPI_TransferCallback) != HAL_OK) {
    ret = MX_WIFI_STATUS_ERROR;
  }

  if (HAL_SPI_RegisterCallback(&MXCHIP_SPI, HAL_SPI_ERROR_CB_ID, MX_WIFI_SPI_ErrorCallback) != HAL_OK) {
    ret = MX_WIFI_STATUS_ERROR;
  }
#endif
...
static int8_t MX_WIFI_SPI_DeInit(void)
{
  ...
  /* Unregister SPI callbacks */
  (void)HAL_SPI_UnRegisterCallback(&MXCHIP_SPI, HAL_SPI_RX_COMPLETE_CB_ID);
  (void)HAL_SPI_UnRegisterCallback(&MXCHIP_SPI, HAL_SPI_TX_COMPLETE_CB_ID);
  (void)HAL_SPI_UnRegisterCallback(&MXCHIP_SPI, HAL_SPI_TX_RX_COMPLETE_CB_ID);
  (void)HAL_SPI_UnRegisterCallback(&MXCHIP_SPI, HAL_SPI_ERROR_CB_ID);
